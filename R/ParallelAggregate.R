...
#' Advanced parallel flowFrame aggregation
#'
#' Creates a flowFrame or matrix with named columns that aggregates data from
#' specified FCS files.
#' The size of this aggregate may be capped at an approximately fixed size.
#' The process uses parallelisation via a SNOW cluster for speed-up.
#'
#' @param fnames character vector. Full paths to FCS files.
#' @param N integer or Inf or NULL. Target aggregate size (>2), or Inf or NULL
#' to take all events. Defaults to NULL.
#' @param batches integer vector or NULL. Batch numbers per FCS file. Defaults
#' to NULL.
#' @param cols integer or character vector or NULL. Indices or names of channels
#' to use, or NULL to use all. Defaults to NULL.
#' @param keep_file_order logical. Whether to keep the given order of FCS files
#' in the aggregate. Defaults to FALSE.
#' @param keep_cell_order logical. Whether to keep the order of cells within
#' each sample in the aggregate. If N is Inf or NULL, implicitly TRUE (due to no
#' subsampling). Defaults to FALSE.
...
#' @param guid character. Value of GUID descriptor if flowFrame is returned.
#' Defaults to 'Aggregate.fcs'.
...
#' onto a specific metacluster, or NULL to not filter events that way. Defaults
#' to NULL.
#' @param metacluster integer. Metacluster number to extract events for if
#' FlowSOM model is specified. Defaults to 1.
#' @param verbose logical. Whether to indicate progress. Defaults to TRUE.
#'
#' @details
#' For a target aggregate size N and n FCS files, each flowFrame contributes
#' about N/n, unless one or more files have fewer than N/n cells, in which case
#' all their events are taken and the sample sizes for other files is increased
...
#'
#' The aggregate gets extra columns: 'File' with index of FCS file of origin per
#' event and 'OriginalIndex' with index of each event within its FCS file of
#' origin.
#'
...
ParallelAggregate <- function(
  ...
  stopifnot('`fnames` must be a non-list vector of multiple paths' =
  ...
  stopifnot('If specified, `cols` must be a vector of column names or indices' =
              ...
              (is.vector(cols) && length(cols)>0 &&
                 (is.integer(cols)||is.character(cols))))
  ...
  if (verbose) {
    ...
    msg <- paste0(msg, ' using ', cores, ' CPU cores')
    message(msg)
  }
  ...
  res <- foreach( # iterate over FCS files asynchronously
    ...
    d <- flowCore::read.FCS(fnames[i])[, cols]@exprs
    if (select_mc) {
      mc_idcs <- which(
  ...
  if (as_flowFrame) {
    ff <- flowCore::read.FCS(fnames[1], which.lines = 1)[, cols]
    ff@exprs <- res[, cols]
    if (note_batch) {
